/*
Public EMMA API

**Base URL:** *<u>https://api.emma.ms/external</u>*  This **Infrastructure API** is for managing the cloud infrastructure within a project.  To access the API, enter your project, navigate to **Settings** > **Service Apps**, and create a service application. Select the access level **Read**, **Operate**, or **Manage**.  After creating the service application, copy the **Client ID** and **Client Secret**. Send an API request to the endpoint **_/issue-token** as specified in the **Authentication** section of the API documentation. You will receive access and refresh tokens in the response.  The Bearer access token is a text string, included in the request header, example:  *-H Authorization: Bearer {token}*  Use this token for API requests. The access token will expire in 10 minutes. A new access token may be created using the refresh token (without Client ID and Client Secret).

API version: 0.0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package emma

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the SecurityGroupRuleRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SecurityGroupRuleRequest{}

// SecurityGroupRuleRequest struct for SecurityGroupRuleRequest
type SecurityGroupRuleRequest struct {
	// Direction of network traffic
	Direction string `json:"direction"`
	// Network protocol
	Protocol string `json:"protocol"`
	// Port
	Ports string `json:"ports"`
	// Range of IPs for rule
	IpRange string `json:"ipRange"`
}

type _SecurityGroupRuleRequest SecurityGroupRuleRequest

// NewSecurityGroupRuleRequest instantiates a new SecurityGroupRuleRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSecurityGroupRuleRequest(direction string, protocol string, ports string, ipRange string) *SecurityGroupRuleRequest {
	this := SecurityGroupRuleRequest{}
	this.Direction = direction
	this.Protocol = protocol
	this.Ports = ports
	this.IpRange = ipRange
	return &this
}

// NewSecurityGroupRuleRequestWithDefaults instantiates a new SecurityGroupRuleRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSecurityGroupRuleRequestWithDefaults() *SecurityGroupRuleRequest {
	this := SecurityGroupRuleRequest{}
	return &this
}

// GetDirection returns the Direction field value
func (o *SecurityGroupRuleRequest) GetDirection() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Direction
}

// GetDirectionOk returns a tuple with the Direction field value
// and a boolean to check if the value has been set.
func (o *SecurityGroupRuleRequest) GetDirectionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Direction, true
}

// SetDirection sets field value
func (o *SecurityGroupRuleRequest) SetDirection(v string) {
	o.Direction = v
}

// GetProtocol returns the Protocol field value
func (o *SecurityGroupRuleRequest) GetProtocol() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Protocol
}

// GetProtocolOk returns a tuple with the Protocol field value
// and a boolean to check if the value has been set.
func (o *SecurityGroupRuleRequest) GetProtocolOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Protocol, true
}

// SetProtocol sets field value
func (o *SecurityGroupRuleRequest) SetProtocol(v string) {
	o.Protocol = v
}

// GetPorts returns the Ports field value
func (o *SecurityGroupRuleRequest) GetPorts() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Ports
}

// GetPortsOk returns a tuple with the Ports field value
// and a boolean to check if the value has been set.
func (o *SecurityGroupRuleRequest) GetPortsOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Ports, true
}

// SetPorts sets field value
func (o *SecurityGroupRuleRequest) SetPorts(v string) {
	o.Ports = v
}

// GetIpRange returns the IpRange field value
func (o *SecurityGroupRuleRequest) GetIpRange() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.IpRange
}

// GetIpRangeOk returns a tuple with the IpRange field value
// and a boolean to check if the value has been set.
func (o *SecurityGroupRuleRequest) GetIpRangeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.IpRange, true
}

// SetIpRange sets field value
func (o *SecurityGroupRuleRequest) SetIpRange(v string) {
	o.IpRange = v
}

func (o SecurityGroupRuleRequest) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SecurityGroupRuleRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["direction"] = o.Direction
	toSerialize["protocol"] = o.Protocol
	toSerialize["ports"] = o.Ports
	toSerialize["ipRange"] = o.IpRange
	return toSerialize, nil
}

func (o *SecurityGroupRuleRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"direction",
		"protocol",
		"ports",
		"ipRange",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varSecurityGroupRuleRequest := _SecurityGroupRuleRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varSecurityGroupRuleRequest)

	if err != nil {
		return err
	}

	*o = SecurityGroupRuleRequest(varSecurityGroupRuleRequest)

	return err
}

type NullableSecurityGroupRuleRequest struct {
	value *SecurityGroupRuleRequest
	isSet bool
}

func (v NullableSecurityGroupRuleRequest) Get() *SecurityGroupRuleRequest {
	return v.value
}

func (v *NullableSecurityGroupRuleRequest) Set(val *SecurityGroupRuleRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableSecurityGroupRuleRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableSecurityGroupRuleRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSecurityGroupRuleRequest(val *SecurityGroupRuleRequest) *NullableSecurityGroupRuleRequest {
	return &NullableSecurityGroupRuleRequest{value: val, isSet: true}
}

func (v NullableSecurityGroupRuleRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSecurityGroupRuleRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
